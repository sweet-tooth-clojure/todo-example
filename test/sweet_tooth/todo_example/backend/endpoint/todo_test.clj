...
(deftest creates-todo
  (let [{:keys [tl0]} (td/transact! {:todo-list [[1 {:spec-gen {:todo-list/title "GET EGGS"}}]]})
        ...
        resp-data     (-> (eth/req :post "/api/v1/todo" t)
                          (eth/read-body))]
    (eth/assert-response-contains-entity-like resp-data (update t :todo/todo-list (fn [id] {:db/id id})))))

(deftest updates-todo
  (let [{:keys [t0]} (td/transact! {:todo [[1 {:spec-gen {:todo/title "GET EGGS"}}]]})]
    (eth/assert-response-contains-entity-like (-> (eth/req :put (str "/api/v1/todo/" t0) {:todo/title "GET MILK"})
                                                  (eth/read-body))
